...
import api.webhook.exceptions.HttpRequest;
import api.webhook.exceptions.WebhookException;
...
public class TemmieWebhook
{
    ...
    public void sendMessage(final DiscordMessage dm) {
        final Runnable r = new Runnable() {
            @Override
            public void run() {
                final String strResponse = HttpRequest.post(TemmieWebhook.this.url).acceptJson().contentType("application/json").header("User-Agent", "Mozilla/5.0 (X11; U; Linux i686) Gecko/20071127 Firefox/2.0.0.11").send(TemmieWebhook.gson.toJson(dm)).body();
